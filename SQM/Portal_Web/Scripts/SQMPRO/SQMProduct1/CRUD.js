$(function () {
    ...
    jQuery("#btnDelete").click(function () {
        ...
        {   //single select
            var RowId = gridDataList.jqGrid('getGridParam', 'selrow');
            if (RowId) {
                var DataKey = (gridDataList.jqGrid('getRowData', RowId)).HRType;
                var r = "ok";
                if (r == "ok") {
                    if (confirm("Confirm to delete selected member (" + gridDataList.jqGrid('getRowData', RowId).HRType + ")?\n\n(Note. Data cannot be recovered once deleted)")) {
                        $.ajax({
                            url: __WebAppPathPrefix + "/SQMPRO/DeleteMember",
                            data: {
                                "HRType": gridDataList.jqGrid('getRowData', RowId).HRType,
                                'CName': gridDataList.jqGrid('getRowData', RowId).CName
                            },
                            type: "post",
                            dataType: 'text',
                            async: false,
