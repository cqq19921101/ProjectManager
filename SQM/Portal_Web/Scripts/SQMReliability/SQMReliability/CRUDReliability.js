$(function () {
    ...
    $("#txtActualTestTime").datepicker({
        ...
        onClose: function (selectedDate) {
            ...
            if (Date.parse(actualTime) >= new Date()) {
                alert("實際試驗時間必須在當前時間之前");
                $(this).val(today);
            }
        }
    });
    ...
    jQuery("#btnReliabilityCreate").click(function () {
        ...
        if (!gridDataList.jqGrid('getGridParam', 'multiselect')) {   //single select
            ...
            ReliMapDialogSetUIByMode(Mapdialog.attr('Mode'));
    ...
    jQuery("#btnReliabilityViewEdit").click(function () {
        ...
        if (!gridDataList.jqGrid('getGridParam', 'multiselect')) {   //single select
            var RowId = gridDataList.jqGrid('getGridParam', 'selrow');
            var Status = gridDataList.jqGrid('getRowData', RowId).Status;
            if (Status == "Pending" || Status == "approve") {
                alert("Pending和approve時不可編輯.");
            } else {
                SetToMapViewMode();
            }
        }
    ...
    function SetToInfoViewMode(SID) {
        //var RowId = gridDataListInfo.jqGrid('getGridParam', 'selrow');
        //if (RowId) {
    ...
    jQuery("#btnReliabilityDelete").click(function () {
        ...
        if (!gridDataList.jqGrid('getGridParam', 'multiselect')) {   //single select
            var RowId = gridDataList.jqGrid('getGridParam', 'selrow');
            if (RowId) {
                var ReliabilitySID = (gridDataList.jqGrid('getRowData', RowId)).ReliabititySID;
                var RowId = gridDataList.jqGrid('getGridParam', 'selrow');
                var Status = gridDataList.jqGrid('getRowData', RowId).Status;
                if (Status == "Pending" || Status == "approve") {
                    alert("Pending和approve時不可編輯.");
                    ...
                    if (true) {
                        if (confirm("Confirm to delete selected ?\n\n(Note. Data cannot be recovered once deleted)")) {
                            $.ajax({
                                url: __WebAppPathPrefix + "/SQMReliability/DeleteReliability",
                                data: { "ReliabilitySID": ReliabilitySID },
                                type: "post",
                                dataType: 'text',
                                async: false,
        ...
        if (!gridDataListInfo.jqGrid('getGridParam', 'multiselect')) {   //single select
            ...
            if (RowId) {
                ...
                if (true) {
                    if (confirm("Confirm to delete selected ?\n\n(Note. Data cannot be recovered once deleted)")) {
                        $.ajax({
                            url: __WebAppPathPrefix + "/SQMReliability/DeleteReliInfo",
    ...
    jQuery("#btnCommitReliability").click(function () {
        ...
        if (!gridDataList.jqGrid('getGridParam', 'multiselect')) {   //single select
            var RowId = gridDataList.jqGrid('getGridParam', 'selrow');
            if (RowId) {
                var ReliabititySID = gridDataList.jqGrid('getRowData', RowId).ReliabititySID;
                //var r = AcquireDataLock(DataKey)
                ...
                if (true) {
                    if (confirm("Confirm to submit?")) {
                        $.ajax({
                            url: __WebAppPathPrefix + "/SQMReliability/ExcelContact",
                            data: {
                                "ReliabilitySID": ReliabititySID,
    ...
    jQuery("#btnCommitFile").click(function () {
        $(this).removeClass('ui-state-focus');
        if (!gridDataList.jqGrid('getGridParam', 'multiselect')) {   //single select
            var RowId = gridDataList.jqGrid('getGridParam', 'selrow');
            if (RowId) {
                var SID = gridDataList.jqGrid('getRowData', RowId).ReliabititySID;
                var FSID = gridDataList.jqGrid('getRowData', RowId).FSID;
                //var r = AcquireDataLock(DataKey)
                ...
                if (true) {
                    if (confirm("Confirm to submit selected?")) {
                        $.ajax({
                            url: __WebAppPathPrefix + "/SQMReliability/CommitFile",
                            data: {
                                "SID": SID,
                                "FSID": FSID
                            },
            ...
            } else { alert("Please select a row data to commit."); }
        }
    });
});