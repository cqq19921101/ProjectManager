function setBasicEvent(fid, type, isValidDate) {
    ...
    if (isValidDate) {
        $('#btnProcess' + fid).click(function () {
            uploadFileECN( fid, type, 'txt' + fid + 'ValidDate');
        });
    } else {
        $('#btnProcess' + fid).click(function () {
            uploadFileECN(fid, type, 'txt' + fid + 'ValidDate');
...
function uploadFileECN(fid, type, validFid) {
    ...
    if ($.get_uploadedFileInfo(ffid).length == 0) {
        ...
        $.ajax({
            url: __WebAppPathPrefix + "/SQMReliability/UploadFileECN",
            data: {
                FA: ReqData,
                Type: type,
            ...
            success: function (data) {
                ...
                if (data != "fail") {
                    ...
                    $("#dialogData").attr(fid + "FGUID", data);
                    getFilesDetail(fid);
                } else {
                    alert(data);
                }
...
function getFilesDetail(fid) {
    ...
    var ECNFile;
    $.ajax({
        url: __WebAppPathPrefix + '/SQMReliability/GetECNFilesDetail',// url: __WebAppPathPrefix + '/VMIBulletin/GetBulletinCategoryList',
        data: {
            "FGUID": escape($.trim($("#dialogData").attr(("" + fid + "FGUID"))))
        },
        type: "post",
        dataType: 'json',
        ...
        success: function (data) {
            if (data.length > 0) {
                ...
                if (ECNFile.FGUID != "null" && ECNFile.FGUID != "undefined") {
                    $("#sp" + fid).html("<a href='" + __WebAppPathPrefix +"/SQMBasic/DownloadSQMFile?DataKey=" + escape($.trim(ECNFile.FGUID)) + "'>" + ECNFile.FileName + "</a>");
                } else {
                    $("#sp" + fid).html("empty");
                }
                //$("#sp" + "IntroUploadTime").html(new Date(ECNFile.IntroTime).yyyymmddhhmmss());
