$(function () {
    ...
    jQuery("#btndialogEditData").click(function () {
        ...
        if (RowId) {
            var r = AcquireDataLock(dialog.attr('RoleGUID'))
            if (r == "ok") {
                dialog.attr('Mode', "e");
                DialogSetUIByMode(dialog.attr('Mode'));
            ...
            if (RowId) {
                ...
                if (r == "ok") {
                    if (confirm("Confirm to delete selected member (" + gridDataList.jqGrid('getRowData', RowId).AccountID + ")?\n\n(Note. Data cannot be recovered once deleted)")) {
                        $.ajax({
                            url: __WebAppPathPrefix + "/SystemMgmt/DeleteRole",
                            data: { "RoleGUID": gridDataList.jqGrid('getRowData', RowId).RoleGUID },
                            type: "post",
                            dataType: 'text',
                            async: false,
