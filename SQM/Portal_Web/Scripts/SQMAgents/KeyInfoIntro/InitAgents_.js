function getSQMAgentsProduct() {
    ...
    $('#btnProcessA').click(function () {
        ...
        if ($.get_uploadedFileInfo('fileupload_uploadVDS').length == 0) {
            ...
            $.ajax({
                url: __WebAppPathPrefix + "/SQMAgents/UploadAgents",
                data: {
                    FA: ReqData,
...
function getFilesDetail() {
    ...
    var BasicInfoGUID = gridDataListBasicInfoType.jqGrid('getRowData', BasicInfoRowId).BasicInfoGUID;
    $.ajax({
        url: __WebAppPathPrefix + '/SQMAgents/GetAgentsDetail',// url: __WebAppPathPrefix + '/VMIBulletin/GetBulletinCategoryList',
        data: {
            "BasicInfoGUID": BasicInfoGUID
        },
        type: "post",
        ...
        success: function (data) {
            if (data.length>0) {
                ...
                if (CritialFile.OfferProxyFGUID != "" && CritialFile.OfferProxyFGUID != "undefine") {
                    $("#sp" + "BasicInfoFile").html("<a href='" + __WebAppPathPrefix + "/SQMAgents/DownloadAgents?DataKey=" + escape($.trim(CritialFile.OfferProxyFGUID)) + "'>" + CritialFile.ProxyFN + "</a>");
                }
            }
