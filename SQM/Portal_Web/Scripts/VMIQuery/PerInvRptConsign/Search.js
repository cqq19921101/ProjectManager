$(function () {
    ...
    $('#btnDownloadPDF').click(function () {
        ...
        var fromSLoc = $.trim($('#txtFromSLoc').val());
        var toSLoc = $.trim($('#txtToSLoc').val());

        if (fromPostDate == '' || toPostDate == '') {
            alert('Please select Post Date(FM) and Post Date(TO) first.');
        }
        else if (dateDiff(fromPostDate, toPostDate) > 32) {
            alert('The interval of the Post Date is exceed 32 days, please reselect it under 32 days.');
        }
        else {
            ...
            $('#tdFromSLoc').text(fromSLoc);
            $('#tdToSLoc').text(toSLoc);

            $.ajax({
                url: __WebAppPathPrefix + '/VMIQuery/GetPeriodInventoryReportPDFFileList',
                data: {
                    ...
                    fromSLoc: escape(fromSLoc),
                    toSLoc: escape(toSLoc),
                ...
                success: function (data) {
                    if (data.length == 0) {
                    ...
                    else if (data[0] == 'The interval of the Post Date is exceed 32 days, please reselect it under 32 days.') {
                        alert(data[0]);
                    }
                    else {
                        var filesLink = '';
                        if (downloadType == 'O') {
                            filesLink = '<a id="OneFile" class="downloadPDF" href="#">Period_Inventory_Report_Consign.pdf</a><br />';
                        }
                        else {
                            $(data).each(function (idx, value) {
                                filesLink += '<a id="' + value + '" class="downloadPDF" href="#">' + value + '_Period_Inventory_Report_Consign.pdf' + '</a><br />';
                            });
                        }
                        ...
                        $('.downloadPDF')
                            ...
                            .click(function () {
                                ...
                                $.ajax({
                                    url: __WebAppPathPrefix + '/VMIQuery/PeriodInventoryReportExportPDF',
                                    data: {
                                        fromVendorCode: escape(fromVendorCode),
                                        toVendorCode: escape(toVendorCode),
                                        fromCompanyCode: escape(fromCompanyCode),
                        ...
                        $('#dialogPerInvRptConsignResult').dialog('open');
                    }
                },
                error: function (xhr, textStatus, thrownError) {
                    $.CommonUIUtility_AjaxErrorHandler(xhr, textStatus, thrownError, __UrlForTimeOut);
