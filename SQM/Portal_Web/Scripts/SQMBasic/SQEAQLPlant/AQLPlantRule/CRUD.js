$(function () {
    ...
    jQuery("#btnShow").click(function () {
        ...
        if (RowId) {   //single select
            ...
            gridDataListMap.jqGrid('setGridParam', { postData: { SID: dataRow.SID } })
            ...
            $("#dialogData").attr('SID', dataRow.SID);
            $("#spInsp").html(dataRow.Name)
            ...
            if (RowId) {
                ...
                if (true) {
                    if (confirm("Confirm to delete selected member (" + gridDataList.jqGrid('getRowData', RowId).SID + ")?\n\n(Note. Data cannot be recovered once deleted)")) {
                        $.ajax({
                            url: __WebAppPathPrefix + "/SQMBasic/DeleteAQLPlantRule",
                            data: {
                                "SID": gridDataList.jqGrid('getRowData', RowId).SID,
                                "SSID": gridDataList.jqGrid('getRowData', RowId).SSID,
                                "SSSID": gridDataList.jqGrid('getRowData', RowId).SSSID
                            },
                            type: "post",
                            dataType: 'text',
                            async: false,
    ...
    jQuery("#btnDisbaled").click(function () {
        $(this).removeClass('ui-state-focus');
        if (!gridDataList.jqGrid('getGridParam', 'multiselect')) {   //single select
            var RowId = gridDataList.jqGrid('getGridParam', 'selrow');
            if (RowId) {
                ...
                //var r = AcquireDataLock(DataKey)
                if (true) {
                    if (confirm("Confirm to disable/enable selected member (" + gridDataList.jqGrid('getRowData', RowId).SID + ")?\n\n")) {
                        $.ajax({
                            url: __WebAppPathPrefix + "/SQMBasic/DisabledAQLPlantRule",
                            data: {
                                "SID": gridDataList.jqGrid('getRowData', RowId).SID,
                                "SSID": gridDataList.jqGrid('getRowData', RowId).SSID,
                                "SSSID": gridDataList.jqGrid('getRowData', RowId).SSSID
                            ...
                            success: function (data) {
                                if (data == "") {
                                    $("#btnSearch").click();
                                    alert("PlantRule disable/enable successfully.");
                                }
                                else {
                                    alert("PlantRule disable/enable fail due to:\n\n" + data);
                                }
                            },
                            error: function (xhr, textStatus, thrownError) {
                                $.CommonUIUtility_AjaxErrorHandler(xhr, textStatus, thrownError, __UrlForTimeOut);
            ...
            } else { alert("Please select a row data to disable."); }
        }
    });
