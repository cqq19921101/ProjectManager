$(function () {
    ...
    jQuery("#btndialogEditData").click(function () {
        ...
        if (RowId) {
            //var r = AcquireDataLock(dialog.attr('spc_id'))
            //if (r == "ok") {
                dialog.attr('Mode', "e");
                DialogSetUIByMode(dialog.attr('Mode'));
    ...
    jQuery("#btnDelete").click(function () {
        ...
        {   //single select
            var RowId = gridDataList.jqGrid('getGridParam', 'selrow');
            if (RowId) {
                var DataKey = (gridDataList.jqGrid('getRowData', RowId)).spc_id;
                //var r = AcquireDataLock(DataKey)
                //if (r == "ok") {
                if (confirm("Confirm to delete selected member (" + gridDataList.jqGrid('getRowData', RowId).spc_id + ")?\n\n(Note. Data cannot be recovered once deleted)")) {
                        $.ajax({
                            url: __WebAppPathPrefix + "/SQECPK/DeleteCPK",
                            data: { "spc_id": gridDataList.jqGrid('getRowData', RowId).spc_id },
                            type: "post",
                            dataType: 'text',
                            async: false,
