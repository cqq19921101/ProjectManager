$(function () {
    ...
    jQuery("#btnDeleteDesc").click(function () {
        ...
        if (!gridDataListDesc.jqGrid('getGridParam', 'multiselect')) {   //single select
            var RowIdCode = gridDataListDesc.jqGrid('getGridParam', 'selrow');
            if (RowIdCode) {
                //var DataKey = (gridDataListDesc.jqGrid('getRowData', RowId)).SubFuncGUID;
                //var r = AcquireDataLock(DataKey)
                if (true) {
                    if (confirm("Confirm to delete selected member (" + gridDataListDesc.jqGrid('getRowData', RowIdCode).Name + ")?\n\n(Note. Data cannot be recovered once deleted)")) {
                        $.ajax({
                            url: __WebAppPathPrefix + "/SQMInsp/DeleteInspMap",
                            data: {
