$(function () {
    ...
    jQuery("#btndialogEditData").click(function () {
        ...
        if (RowId) {
            var r = AcquireDataLock(dialog.attr('AccountGUID'))
            if (r == "ok") {
                dialog.attr('Mode', "e");
                DialogSetUIByMode(dialog.attr('Mode'));
            ...
            if (RowId) {
                ...
                if (r == "ok") {
                    if (confirm("Confirm to delete selected member (" + gridDataList.jqGrid('getRowData', RowId).AccountID + ")?\n\n(Note. Data cannot be recovered once deleted)")) {
                        $.ajax({
                            url: __WebAppPathPrefix + "/SubSystemMgmt/DeleteMember",
                            data: { "AccountGUID": gridDataList.jqGrid('getRowData', RowId).AccountGUID },
                            type: "post",
                            dataType: 'text',
                            async: false,
                            success: function (data) {
                                if (data == "") {
                                    ...
                                    alert("Member delete fail due to:\n\n" + data);
                                }
                            },
