...
function uploadFile(fid) {
    ...
    if ($.get_uploadedFileInfo(ffid).length == 0) {
        ...
        $.ajax({
            url: __WebAppPathPrefix + "/SQMMRB/UploadFile",
            data: {
                FA: ReqData
            },
            type: "post",
...
function getFilesDetail(fid) {
    ...
    var MRBFile;
    $.ajax({
        url: __WebAppPathPrefix + '/SQMMRB/GetMRBFilesDetail',// url: __WebAppPathPrefix + '/VMIBulletin/GetBulletinCategoryList',
        data: {
        ...
        success: function (data) {
            if (data.length > 0) {
                ...
                if (MRBFile.FGUID != "null" && MRBFile.FGUID != "undefined") {
                    $("#sp" + fid).html("<a href='" + __WebAppPathPrefix + "/SQMBasic/DownloadSQMFile?DataKey=" + escape($.trim(MRBFile.FGUID)) + "'>" + MRBFile.FileName + "</a>");
                } else {
                    $("#sp" + fid).html("empty");
                }
                //$("#sp" + "IntroUploadTime").html(new Date(MRBFile.IntroTime).yyyymmddhhmmss());
