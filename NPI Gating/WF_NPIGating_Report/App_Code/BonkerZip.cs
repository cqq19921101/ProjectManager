 ...
 public class BonkerZip
    {
        ...
        /// <returns></returns>
        public Dictionary<string, int> DeCompressionZip(string _depositPath, string _floderPath)
        {
            ...
            try
            {
                ...
                while (ze != null)//如果解压完ze则是null
                {
                    if (ze.IsFile)//压缩zipINputStream里面存的都是文件。带文件夹的文件名字是文件夹\\文件名
                    {
                        ...
                        fs = new FileStream(_floderPath + "\\" + ze.Name.Substring(ze.Name.LastIndexOf("/") + 1, ze.Name.Length - ze.Name.LastIndexOf("/") - 1), FileMode.OpenOrCreate, FileAccess.Write);//创建文件
                        //循环读取文件到文件流中
                        while (true)
                        {
